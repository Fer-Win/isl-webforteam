...
const PlayerCard: React.FC<PlayerCardProps> = ({  image, name, stats }) => {
  return (
    <a href={`/player/${name}`} className='group hover:rounded-md'>
     ...
     <div className='flex flex-col px-3 py-2 text-white bg-[#013ca6] border-2 border-white font-familjen group-hover:rounded-b-md transition-all ease-in duration-300'>
     <h1 className='text-xl align-middle text-center font-bold'>{name}</h1>
      <ul>
